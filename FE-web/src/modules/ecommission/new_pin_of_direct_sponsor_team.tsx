...
const NewPinOfDirectSponsorTeam = ({
  list,
  loading,
  header,
}: {
    ...
    <div className="md:mx-auto md:w-1216 relative mb-8 px-3 md:px-0">
      <div className="md:flex mt-4 md:mt-6">
        ...
        <div className="w-full md:pl-20 md:flex-grow">
          <p className="text-sm font-medium md:w-full">{t("new_pin_of_direct_sponsor_team")}</p>
          <div className="mt-5">
            ...
            {!loading && header.length > 0 && (
              <div className="flex bg-orangeLight h-8 items-center rounded pl-3 w-full pr-3 mt-2">
                ...
                <div
                  className={`text-sm grid grid-cols-${headerLength} text-orange`}
                  style={{ width: "85%" }}
                >
                  {header.map((item) => (
...
export default NewPinOfDirectSponsorTeam;
