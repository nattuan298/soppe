...
import CommissionReport from "src/modules/ecommission/commission-report";
...
export default function CommissionReportPage() {
  ...
  const [list, setList] = useState<CommissionReportType[] | undefined>();
  ...
  useEffect(() => {
    const callAPI = async () => {
      setLoading(true);
      try {
        const api = `${apiRoute.members.commissionReport}?startDate=${dayjs(from).format(
          "YYYY-MM-DD",
  ...
  return (
    <div className="w-full text-black-dark">
      <NextSeo title={t`commission_report`} />
      <main>
        <HeaderSignIn
