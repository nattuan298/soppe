...
import CycleHistory from "src/modules/ecommission/cycle-history";
...
export default function CycleHistoryReportPage() {
  ...
  useEffect(() => {
    const callAPI = async () => {
      setLoading(true);
      try {
        const api = `${apiRoute.agentReports.cycleHistory}?startDate=${dayjs(from).format(
          "YYYY-MM-DD",
  ...
  return (
    <div className="w-full text-black-dark">
      <NextSeo title={t`cycle_history_report`} />
      <main>
        <HeaderSignIn
          title={t`cycle_history_report`}
          showDateRange
