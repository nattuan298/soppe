...
function Option({ onSelect, isSeleted, ...item }: OptionProps & CitizenCodeType) {
  ...
  return (
    <li
      className={`${
        isSeleted && "bg-white text-orange-light"
      } hover:bg-orange-light hover:text-white cursor-pointer h-50 flex items-center p-3.5`}
...
interface SelectProps {
  ...
  disabled?: boolean;
  className?: string;
  options?: CountryPhoneCodeType[];
}

export function SelectCountry2({ country, onSelect, disabled, className, options }: SelectProps) {
  const { t } = useTranslation("common");
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const selectRef = useRef<HTMLDivElement>(null);
  // const ListCountryPhoneCode = useListCountryPhoneCode();
  const [value, setValue] = useState<CitizenCodeType>(
    (options || ListCountryPhoneCode).find(
      (item: CountryPhoneCodeType) => item.value === country,
    ) || {
      name: "Thailand",
      code: "66",
  ...
  useEffect(() => {
    setValue(
      (options || ListCountryPhoneCode).find((item) => item.value === country) || {
        name: "Thailand",
        code: "66",
        flag: "/assets/images/country/thailand.svg",
        value: "Thailand",
  ...
  // const name = useMemo(() => t(value.name.toLocaleLowerCase() as "to_ship"), [t]);
    ...
    <div className={clsx(selectClassName, "country-select relative h-50 select-custom")}>
      <div
        className={`select flex items-center justify-between pr-2 pl-3 w-full ${
          disabled ? "bg-graySearch-light" : "bg-white"
        }`}
        role="button"
      ...
      >
        {(options || ListCountryPhoneCode).map((option) => (
          <Option
            key={option.value}
