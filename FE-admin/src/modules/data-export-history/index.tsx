...
import { fetchDataExportHistoryList } from "src/store/data-export-history.action";
import { getLinkDataExportHistory } from "src/services/export-data.services";

const queryString = require("query-string");

export default function DataExportHistoryList() {
  const date = new Date();
  ...
  const { DataExportHistoryData, loading, errorMessage } = useSelector(
    (state: RootState) => state.dataExportHistory,
  );
  const locationSearch = queryString.parse(location.search);
  const getData = useCallback(async () => {
    dispatch(
      fetchDataExportHistoryList({
        page,
        pageSize,
        keyword: search.name,
  ...
  useEffect(() => {
    if (DataExportHistoryData?.data?.length === 0 && !loading && errorMessage) {
      return;
    }
  }, [DataExportHistoryData, loading, errorMessage, t]);
  ...
  return (
    <div className="access-history-list">
      ...
      <div className="faq-table bg-white">
        <Table>
          <TableHead>
            <CollapsibleHeadRow>
              ...
              <TableCell>{t("exported_data")}</TableCell>
              <TableCell>{t("start_date")}</TableCell>
              <TableCell>{t("end_date")}</TableCell>
              <TableCell>{t("exported_date")}</TableCell>
              <TableCell />
            </CollapsibleHeadRow>
          </TableHead>
          <TableBody>
            {loading ? (
              ...
              DataExportHistoryData?.data?.map((history: DataExportModel) => (
                <CollapsibleBodyRow
                  colSpan={9}
                  key={history._id}
                  preview={
                ...
                >
                  ...
                  <TableCell>{history.exportedData}</TableCell>
                  <TableCell className="max-w-[150px]">
                    <div className="max-w-[120px]">
                      {history.startDate && dayjs(history.startDate).format("DD MMM YYYY HH:mm:ss")}
                    </div>
                  </TableCell>
                  <TableCell className="max-w-[150px]">
                    <div className="max-w-[120px]">
                      {history.endDate && dayjs(history.endDate).format("DD MMM YYYY HH:mm:ss")}
                    </div>
                  </TableCell>
                  <TableCell className="max-w-[150px]">
                    <div className="max-w-[120px]">
            ...
            {!DataExportHistoryData?.data?.length && !loading && (
              <TableRow>
        ...
        <Pagination
          totalPage={Math.ceil(DataExportHistoryData?.total / DataExportHistoryData?.limit)}
