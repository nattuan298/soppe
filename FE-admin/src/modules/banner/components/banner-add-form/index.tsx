...
import { fetchBannerSectionSelected } from "src/store/internal-banners.action";
import { InFormationBanner } from "./banner-information";
...
export function AddBannerForm({ isEditMode }: PropsType) {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      dispatch(fetchBannerSectionSelected(id));
  ...
  const createBanner = async (values: any) => {
    ...
    try {
      const response = await createInternalBannerService(body);
  ...
  const editBanner = async (values: any) => {
    ...
    try {
      const response = await editInternalBannerService(editBanner);
      if (!response?.statusCode) {
        history.push(
          `/admin-dashboard/appearance-management/home-banner-management/banner-loop-list/banner-list/${selectedBanner.bannerLoop}`,
        );
      }
      setIsSubmiting(false);
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    setStatus(selectedBanner?.status === "ACTIVE" ? "1" : "2");
  }, [selectedBanner]);
  ...
  const handleConfirm = () => {
    ...
    history.push(
      `/admin-dashboard/appearance-management/home-banner-management/banner-loop-list/banner-list/${idBannerLoop}`,
    );
  };
        ...
        <div className="flex items-start">
          <div className="w-full flex-1 items-start">
            ...
            <div className="button-wapper px-5 mb-10 flex items-center">
              ...
              <Button
                variant="text"
                className="bg-white border button-cancel h-12 w-72 ml-4"
                onClick={openModal}
              >
                {t`cancel`}
              </Button>
