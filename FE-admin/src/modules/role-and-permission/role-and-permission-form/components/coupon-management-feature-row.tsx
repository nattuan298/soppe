...
import { setCouponManagementFeaturePermission } from "src/store/permission.slice";
...
interface CouponManagementFeatureRowProps {
  defaultValue?: {
...
export default function CouponManagementFeatureRow({
  defaultValue,
  value,
}: CouponManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(setCouponManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("coupon-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
