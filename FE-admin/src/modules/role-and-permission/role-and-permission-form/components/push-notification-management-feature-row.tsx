...
import { getPushNotificationManagementFeaturePermission } from "src/store/permission.slice";
import { CollapsibleBodyRow, RoundedCheckbox } from "src/components";
import { PermissionModel } from "src/types/permission.model";
import { useStyles } from "./styles";

interface PushNotificationManagementFeatureRowProps {
  defaultValue?: {
...
export default function PushNotificationManagementFeatureRow({
  defaultValue,
  value,
}: PushNotificationManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getPushNotificationManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("push-notification-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
