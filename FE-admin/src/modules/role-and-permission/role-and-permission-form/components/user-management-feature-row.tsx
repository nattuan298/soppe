...
export default function UserManagementFeatureRow({
  defaultValue,
  value,
}: UserManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getUserManagementFeaturePermissions(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("user-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
