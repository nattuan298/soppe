...
import { getCategoriesManagementFeaturePermission } from "src/store/permission.slice";
...
interface CategoriesManagementFeatureRowProps {
  defaultValue?: {
...
export default function CategoriesManagementFeatureRow({
  defaultValue,
  value,
}: CategoriesManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getCategoriesManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("categories-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
