...
import { getInternalUserManagementFeaturePermission } from "src/store/permission.slice";
import { CollapsibleBodyRow, RoundedCheckbox } from "src/components";
import { PermissionModel } from "src/types/permission.model";
import { useStyles } from "./styles";

interface InternalUserManagementFeatureRowProps {
  defaultValue: {
...
export default function InternalUserManagementFeatureRow({
  defaultValue,
  value,
}: InternalUserManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getInternalUserManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("internal-user-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
