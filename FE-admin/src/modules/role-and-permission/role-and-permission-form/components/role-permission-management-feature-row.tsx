...
import { getRolePermissionManagementFeaturePermission } from "src/store/permission.slice";
import { CollapsibleBodyRow, RoundedCheckbox } from "src/components";
import { PermissionModel } from "src/types/permission.model";
import { useStyles } from "./styles";
interface RolePermissionManagementFeatureRowProps {
  defaultValue?: {
...
export default function RolePermissionManagementFeatureRow({
  defaultValue,
  value,
}: RolePermissionManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getRolePermissionManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("role-permission-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
