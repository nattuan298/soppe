...
import { getNewsArticleManagementFeaturePermission } from "src/store/permission.slice";
import { CollapsibleBodyRow, RoundedCheckbox } from "src/components";
import { PermissionModel } from "src/types/permission.model";
import { useStyles } from "./styles";

interface NewsArticleManagementFeatureRowProps {
  defaultValue: {
...
export default function NewsArticleManagementFeatureRow({
  defaultValue,
  value,
}: NewsArticleManagementFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getNewsArticleManagementFeaturePermission(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("news-article-management-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
