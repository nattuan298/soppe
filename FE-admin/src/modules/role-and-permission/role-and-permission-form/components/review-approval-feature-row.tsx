...
export default function ReviewApprovalFeatureRow({
  defaultValue,
  value,
}: ReviewApprovalFeatureRowProps) {
  ...
  useEffect(() => {
    dispatch(getReviewApprovalFeaturePermissions(permission));
  }, [dispatch, permission]);
  ...
  return (
    <CollapsibleBodyRow className={row} collapsible={false} borderedPreview={false}>
      <TableCell>{t("review-approval-feature")}</TableCell>
      <TableCell>
        <RoundedCheckbox
